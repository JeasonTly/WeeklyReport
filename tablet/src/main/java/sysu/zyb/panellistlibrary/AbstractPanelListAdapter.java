...
public abstract class AbstractPanelListAdapter {
    ...
    private MyHorizontalScrollView jihuagongshi_view;
    ...
    private TextView jihua_title;//计划工时
    private LinearLayout ll_row;//上方的表头
    private LinearLayout jihua_row;//计划工时
     ...
     */
    public AbstractPanelListAdapter(Context context, PanelListLayout pl_root, ListView lv_content,WorkTimePlanClickListener workTimePlanClickListener) {
    ...
    public void setPlanDataList(List<String> planDataList) {
       ...
       // ((DefaultContentAdapter)contentAdapter).refreshPlanList(planDataList);
    }
     ...
     * 初始化总Adapter，加载数据到视图
     */
    private DefaultContentAdapter getDefaultContentAdapter(){
         DefaultContentAdapter contentAdapter = new DefaultContentAdapter(context, R.layout.defaultcontentitem,
                    contentDataList, itemWidthList, planDataList, itemHeight, lv_content);
    ...
    void initAdapter() {
        ...
        jihuagongshi_view.setOnHorizontalScrollListener(horizontalScrollListener);
    ...
    private void reorganizeViewGroup() {
        ...
        if(contentAdapter == null){
            contentAdapter = getDefaultContentAdapter();
        }
...
//        lv_content.setFastScrollEnabled(false);
        ...
        // 1. title (TextView --> PanelListLayout)
        jihua_title = new TextView(context);
        jihua_title.setText("计划工时");
        if (titleBackgroundResource != 0) {
            jihua_title.setBackgroundResource(titleBackgroundResource);
        }
        jihua_title.getPaint().setFakeBoldText(true);
        jihua_title.setGravity(Gravity.CENTER);
        jihua_title.setBackground(ContextCompat.getDrawable(context, R.drawable.base_line_cf));
        //jihua_title.setTextColor(Color.B);
        jihua_title.setId(View.generateViewId());//设置一个随机id，这样可以保证不冲突
        //RelativeLayout.LayoutParams lp_tv_title = new RelativeLayout.LayoutParams(titleWidth, titleHeight);
        RelativeLayout.LayoutParams lp_jihua_title = new RelativeLayout.LayoutParams(titleWidth, titleHeight);
        lp_jihua_title.addRule(RelativeLayout.BELOW, tv_title.getId());
        pl_root.addView(jihua_title, lp_jihua_title);



        jihua_row = new LinearLayout(context);
        jihua_row.setLayoutParams(lp);
        jihuagongshi_view = new MyHorizontalScrollView(context);
        jihuagongshi_view.setHorizontalScrollBarEnabled(false);
        jihuagongshi_view.setOverScrollMode(View.OVER_SCROLL_NEVER);//去除滑动到边缘时出现的阴影
        jihuagongshi_view.addView(jihua_row);// 暂时先不给ll_row添加子view，等布局画出来了再添加
        jihuagongshi_view.setId(View.generateViewId());
        RelativeLayout.LayoutParams lp_jihuagongshi = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, titleHeight);
        lp_jihuagongshi.addRule(RelativeLayout.END_OF, jihua_title.getId());
        lp_jihuagongshi.addRule(RelativeLayout.BELOW, mhsv_row.getId());
        pl_root.addView(jihuagongshi_view, lp_jihuagongshi);
        ...
        lp_lv_column.addRule(RelativeLayout.BELOW, jihuagongshi_view.getId());
...
//        Log.d(TAG, " reorganizeViewGroup: " + onRefreshListener.toString());
        ...
        lp_mhsv_content.addRule(RelativeLayout.BELOW, jihua_title.getId());
...
//        }
     ...
     * must be called in pl_root.post();
     */
    private void initJihuaLayout() {

        Integer[] widthArray = new Integer[getPlanDataList().size()];
        Log.d(TAG," 初始化计划列表 "+ (ll_contentItem == null ) +" widthArray 长度"+widthArray.length);
        if (ll_contentItem == null) {
            if (itemWidthList != null) {
                for (int i = 0; i < widthArray.length; i++) {
                    widthArray[i] = itemWidthList.get(i);
        ...
        List<String> rowDataList1 = getPlanDataList();
        int rowCount = rowDataList1.size();

        // jihua_row.setBackgroundColor(Color.parseColor(rowColor));
        //分隔线的设置，如果content的item设置了分割线，那row使用相同的分割线，除非单独给row设置了分割线
        if (rowDivider == null) {
            if (ll_contentItem != null) {
                jihua_row.setDividerDrawable(ll_contentItem.getDividerDrawable());
                jihua_row.setShowDividers(LinearLayout.SHOW_DIVIDER_MIDDLE);
            }
        } else {
            jihua_row.setDividerDrawable(rowDivider);
        }
        jihua_row.removeAllViews();
        Log.d(TAG," initJihuaLayout " + rowCount + " rowDataList1 "+rowDataList1.toString());
        // 横向表头每一个 item 的宽度都取决于 content 的 item 的宽度
        for (int i = 0; i < rowCount; i++) {
            TextView rowItem = new TextView(context);
            rowItem.setText(rowDataList1.get(i));//设置文字
            ...
            rowItem.setWidth(widthArray[i]);//设置宽度
            rowItem.setHeight(titleHeight);//设置高度
            rowItem.setTextColor(ContextCompat.getColor(context, R.color.colorColumn));
            rowItem.setGravity(Gravity.CENTER);
            rowItem.setBackground(ContextCompat.getDrawable(context, R.drawable.base_line_cf));
            final int position = i;
            rowItem.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    workTimePlanClickListener.workTimePlanClick(position);
                }
            });
            jihua_row.addView(rowItem);
        }
    ...
    public void scrollToDefaultPostion(int position){
        ...
        jihuagongshi_view.smoothScrollTo(position*itemWidthList.get(0),0);
    }
    /**
    ...
    private class HorizontalScrollListener implements MyHorizontalScrollView.OnHorizontalScrollListener {
        ...
        public void onHorizontalScrolled(MyHorizontalScrollView view, int l, int t, int oldl, int oldt) {
            if (view == mhsv_content) {
               // Log.d(TAG, " mhsv_content scrollTo");
                Log.d(TAG," scroll t is "+t +" left is "+l);
                mhsv_row.scrollTo(l, t);
                jihuagongshi_view.scrollTo(l, t);
            } else {
